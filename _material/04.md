...
Tutustumme tässä luvussa hajautusta käyttävien tietorakenteiden ominaisuuksiin ja niiden taustalla olevaan teoriaan. Näemme myös, miten näitä tietorakenteita voidaan käyttää tehokkaiden algoritmien suunnittelussa.
...
Tämä on kuitenkin hidas algoritmi, koska se selvittää luvun sijainnin listassa metodilla `index`, joka vie aikaa $$O(n)$$. Tämän vuoksi algoritmin aikavaativuus on $$O(n^2)$$.

Voimme kuitenkin toteuttaa saman idean tehokkaan ottamalla käyttöön sanakirjan, joka sisältää kunkin luvun kohdan listassa:
...
Nyt algoritmissa on kaksi silmukkaa, jotka toimivat molemmat ajassa $$O(n)$$, koska hajautus on tehokasta. Niinpä koko algoritmin aikavaativuus on $$O(n)$$.
...
Huomaa, että hajautustaulun koko $$N$$ ei ole sama asia kuin tietorakenteen alkioiden määrä $$n$$. Hajautustaulun koko valitaan niin, että siinä on sopivasti tilaa alkioille.
...
Oletetaan seuraavaksi, että hajautustaulun kokona on $$N=8$$. Pythonissa hajautusfunktio on `hash`, joka antaa hajautusarvoksi 64-bittisen kokonaisluvun. Funktiota voidaan käyttää näin:
...
Alkion sijainti hajautustaulussa saadaan jakojäännöksen avulla:
...
Hajautustaulu on tehokas, koska alkion hajautusarvo kertoo sen sijainnin hajautustaulussa. Kun alkio lisätään joukkoon, se lisätään hajautusarvon mukaiseen kohtaan hajautustaulussa. Vastaavasti kun halutaan tarkastaa alkion kuuluminen joukkoon, voidaan laskea sen hajautusarvo ja tutkia, onko alkio hajautustaulussa vastaavassa kohdassa.

Jos oletetaan, että jokaisella alkiolla on eri hajautusarvo, hajautustaulun käyttäminen on helppoa ja alkion lisääminen, hakeminen ja poistaminen voidaan toteuttaa $$O(1)$$-ajassa. Käytännössä näin ei kuitenkaan voida olettaa, vaan hajautuksessa voi tulla _törmäys_ eli kahdella eri alkiolla on sama hajautusarvo.

### Törmäysten käsittely

Hajautuksessa törmäykset ovat väistämättömiä, koska eri alkioiden määrä on yleensä suurempi kuin hajautustaulun koko $$N$$. Esimerkiksi kun hajautustauluun lisätään merkkijonoja, erilaisia merkkijonoja on rajattomasti eikä ole mahdollista, että jokaisella merkkijonolla olisi eri hajautusarvo väliltä $$0 \dots N-1$$.
