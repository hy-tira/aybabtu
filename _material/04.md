...
Pythonin tietorakenteiden käyttämä hajautusfunktio on `hash`, joka antaa hajautusarvoksi 64-bittisen kokonaisluvun. Funktiota voidaan käyttää näin:
...
Oletetaan seuraavaksi, että hajautustaulun kokona on $$N=8$$. Alkion sijainti hajautustaulussa saadaan hajautusarvosta jakojäännöksen avulla:
...
Hajautustaulun tehokkuus perustuu siihen, että alkion hajautusarvo kertoo sen paikan hajautustaulussa. Esimerkiksi jos halutaan tarkastaa, kuuluuko merkkijono `"apina"` yllä olevaan joukkoon, hajautusarvon perusteella tiedetään, että sen tulisi olla hajautustaulussa kohdassa 6.
...
Tässä merkkijonoilla `"apina"` ja `"dyyni"` on sama paikka hajautustaulussa, koska niiden hajautusarvojen jakojäännös $$N$$:llä on sama. Tällainen tilanne on _törmäys_ (_collision_) ja hajautustaulun toteutuksessa tulee varautua asiaan.

Yksi tapa käsitellä törmäykset on toteuttaa alkion paikan valinta niin, että jos hajautusarvon mukainen paikka on jo toisen alkion käytössä, alkiolle aletaan etsiä uutta paikkaa sopivan matemaattisen kaavan avulla. Esimerkiksi Pythonissa hajautustaulun toteutus perustuu [tällaiseen ideaan](../todo).

Käytännössä törmäyksiä ei esiinny yleensä paljon, jos hajautustaulun koko ja törmäysten käsittelytapa on valittu sopivasti. Tämän ansiosta voidaan olettaa, että hajautustaulun operaatiot toimivat $$O(1)$$-ajassa.
