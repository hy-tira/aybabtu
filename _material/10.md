...
Dynaamisen ohjelmoinnin käyttäminen voi aiheuttaa paljon sisäkkäistä rekursiota, joka voi aiheuttaa ongelman koodin suorittamiseen. Esimerkiksi seuraava koodi ei toimi halutulla tavalla:

```python
print(count_coins(1000)) # ?
```

Koodin suoritus aiheuttaa virheen "RecursionError: maximum recursion depth exceeded in comparison" mikä tarkoittaa, että funktiota `count_coins` kutsutaan liian monta kertaa rekursiivisesti.
...
```python
print(count_coins(1000)) # 3087740630867966143402129656887650927057799620274201531584941705795516190845145228592854267342184573035100911507977364354216196288107850787121712648914858264769299599979122374039772390518895813332423815694843710978676714986398419599
```

Toinen vaihtoehto on toteuttaa dynaaminen ohjelmointi eri tavalla ilman rekursiota. Esimerkiksi funktio `count_coins` voidaan toteuttaa seuraavasti käyttäen rekursion sijasta silmukkaa:
...
Tässä sanakirja `result` sisältää vastaukset osaongelmiin. Funktio merkitsee aluksi sanakirjaan, että tapauksen $$x=0$$ vastaus on $$1$$. Tämän jälkeen pääsilmukka käy läpi osaongelmat tapauksissa $$1,2,\dots,x$$ ja laskee kunkin osaongelman vastauksen aiemmin laskettujen tulosten avulla.

Tämän toteutuksen etuna on, että koodissa ei ole rekursiota eikä siinä voi tulla liikaa rekursiivisia kutsuja. Huonona puolena monimutkaisempia dynaamisen ohjelmoinnin ratkaisuja voi olla vaivalloista toteuttaa ilman rekursiota, koska silloin täytyy suunnitella itse, missä järjestyksessä osaongelmat tulee ratkaista.
...
Joskus raa'an voiman ratkaisu on yllättävän helppoa muuttaa dynaamisen ohjelmoinnin ratkaisuksi. Näin on esimerkiksi seuraavassa tehtävässä:
...
Tämän ratkaisun voi muuttaa suoraan tehokkaaksi dynaamisen ohjelmoinnin ratkaisuksi tallentamalla tulokset muistiin:
...
def count_sequences(n, level=0):
    ...
    cache[(n, level)] = count_sequences(n - 1, level + 1) + \
                        count_sequences(n - 1, level - 1)
    return cache[(n, level)]
```

Tämän muutoksen jälkeen voidaan laskea tehokkaasti esimerkiksi tapaus $$n=100$$:
...
Tässä on vielä toinen dynaamisen ohjelmoinnin algoritmi, jossa funktiolla on vain yksi parametri:
...
Tässä ideana on käydä läpi kaikki tavat valita kohta, jossa on ensimmäistä alkusulkua `(` vastaava loppusulku `)`. Jokaisessa tavassa näiden sulkujen väliin sekä näiden sulkujen jälkeen tulee jokin sulkulauseke, minkä takia lausekkeiden määrä saadaan kertomalla väliin ja jälkeen tulevien lausekkeiden määrät.
