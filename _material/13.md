...
Suunnatun verkon _vahvasti yhtenäinen komponentti_ (_strongly connected component_) on joukko solmuja, jossa mistä tahansa solmusta on polku mihin tahansa solmuun. Verkko voidaan jakaa aina vahvasti yhtenäisiin komponentteihin. Tarkastellaan esimerkkinä seuraavaa verkkoa:

![](../vahyht1.png)

Tässä tapauksessa vahvasti yhtenäiset komponentit ovat seuraavat:
...
Tällainen komponenteista muodostuva verkko on aina syklitön ja se näyttää verkon syvärakenteen: miten verkossa pystyy liikkumaan solmuista toisiinsa.
...
Algoritmin toinen vaihe suoritetaan _käänteisessä_ verkossa, jossa jokaisen kaaren suunta on käänteinen. Algoritmi käy läpi solmujen listan käänteisessä järjestyksessä ja aloittaa haun jokaisesta solmusta, jota ei ole vielä käsitelty. Jokainen tällainen haku muodostaa yhden vahvasti yhtenäisen komponentin.
...
class Kosaraju:
    def __init__(self, n):
        self.n = n
        self.graph = {node: [] for node in range(1, n + 1)}
        self.reverse = {node: [] for node in range(1, n + 1)}
...
Kosarajun algoritmin toisessa vaiheessa algoritmi lisää vahvasti yhtenäiseen komponenttiin kaikki solmut, joihin pääsee alkusolmusta ja joita ei ole vielä lisätty toiseen komponenttiin. Miten on varmaa, että tuloksena on vahvasti yhtenäinen komponentti eikä siihen tule ylimääräisiä solmuja?
