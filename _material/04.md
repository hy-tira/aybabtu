...
Huomaa, että hajautustaulun koko $$N$$ ei ole sama asia kuin tietorakenteen alkioiden määrä $$n$$. Hajautustaulun koko valitaan niin, että siinä on riittävästi tilaa alkioille.
...
Muuttumattomia olioita ovat esimerkiksi luvut, merkkijonot ja tuplet, joissa jokainen jäsen on muuttumaton. Niille voi laskea hajautusarvon:
...
Omasta luokasta saa hajautettavan lisäämällä siihen metodin `__hash__`, joka palauttaa hajautusarvon. Metodi voi toimia esimerkiksi niin, että se antaa olion sisältöä vastaavan tuplen hajautusarvon:
...
print(hash(Location(3, -5))) # -3089947874767036032
```

Huomaa, että hajautus ei toimi oikein, jos metodia `__hash__` ei määrittele. Tällöin kahdella eri oliolla voi olla eri hajautusarvo, vaikka niiden sisältö olisi sama:
...
print(hash(a)) # 8742459605486
print(hash(b)) # 8742459594650
```
