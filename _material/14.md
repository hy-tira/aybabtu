...
Luokassa lista `nodes` sisältää verkon solmut ja lista `edges` sisältää verkon kaaret. Sanakirja `distances` sisältää etäisyydet alkusolmusta verkon solmuihin. Joka kierroksella algoritmi käy läpi listan `edges` ja koettaa parantaa etäisyyksiä kaarten avulla. Jos uusi etäisyys on lyhempi kuin vanha etäisyys, algoritmi päivittää sen sanakirjaan `distances`.
